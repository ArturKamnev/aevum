...
**Aevum** — это приложение для планирования задач на Windows, в котором обычный todo-list объединён с AI-функциями, календарным планированием, визуализацией дня и напоминаниями.
...
Aevum поможет превратить это в удобные задачи с датой, временем, длительностью и повторением.
...
Aevum умеет напоминать о запланированных задачах прямо на компьютере.
...
```mermaid
flowchart LR
    User[Пользователь] --> App[Aevum Interface]
...
1. Откройте страницу последней версии:
   ...
   [Скачать Aevum](https://github.com/ArturKamnev/todo-list/releases/latest)

2. В разделе **Assets** скачайте файл:

```text
Aevum Setup x.x.x.exe
```

3. Запустите скачанный установщик.

4. После установки откройте **Aevum**.
...
Aevum поддерживает автоматические обновления.
...
Aevum всё ещё развивается. В будущих версиях могут появиться:
